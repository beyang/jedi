...
class Evaluator(object):
    ...
    def find_types(self, scope, name_str, position=None, search_global=False,
        ...
        :param follow_statements: Whether to resolve statements (i.e. variables) to their type.
               If False, will resolve variables only to the variable definition, not to the type of the variable.
    ...
    def _has_next(self, path):
        """
        Helper function to check if iterable is empty.
        Usage: path, has_next = _has_next(path)
        Note: you must reassign the argument variable, because the original iterable is modified.
        """
    ...
    def eval_call_path(self, path, scope, position, force_resolve_vars_to_types=False):

        # Kludge to force resolution of name to type
        if force_resolve_vars_to_types:
            resolve_variables_to_types = True
        else:
            resolve_variables_to_types = self.resolve_variables_to_types

        """
        Follows a path generated by `pr.StatementElement.generate_call_path()`.
        """
        current = next(path)
        path, path_has_next = self._has_next(path)
